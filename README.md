...
## Projektportfölj-verktyg (MVP) – projektbrief

- **Syfte**: registrera projekt (start/slut, projektägare, projektledare, effektägare), skapa aktiviteter och beslutspunkter, koppla beroenden mellan aktiviteter och visa en enkel portföljvy över alla projekt.
- **Arkitektur**: frontend i React (Vite + TypeScript), backend/data i Supabase (PostgreSQL + Auth) på gratisnivån; ingen egen server i första versionen; hostas på GitHub Pages eller Netlify med CI/CD kopplat till repo.
- **Datamodell (MVP)**:
  - `users`: id, email, name, role (t.ex. admin/projektledare/läsare).
  - `projects`: id, name, description, start_date, end_date, project_owner, project_manager, impact_owner, status, priority.
  - `activities`: id, project_id (FK → projects.id), name, description, start_date, end_date, status, responsible.
  - `milestones`: id, project_id (FK → projects.id), name, decision_type, date, status.
  - `dependencies`: id, from_activity_id (FK → activities.id), to_activity_id (FK → activities.id), type (finish-to-start i första läget).
- **Funktioner i MVP**: inloggning via Supabase Auth, projektlista/portföljvy med statusfilter, projektdetaljsida med aktiviteter & beslutspunkter, CRUD-formulär, enkla beroenden inom samma projekt, deployment via Netlify/GitHub Pages.
- **Tidsuppskattning (MVP 1)**: ca 60–120 timmar (setup 8–15h, datamodell 4–8h, auth 8–15h, projekt-CRUD 10–15h, aktiviteter/beslut 10–20h, portföljvy 8–15h, deploy/småfix 8–15h).
- **Namnidéer (.se)**: portfoljkollen.se · portfoljpanelen.se · projektflode.se · projektkompassen.se · projektkedjan.se · projfolio.se · flowfolio.se · impactflow.se
...
1. Klona repot och installera beroenden:
   ...
   npm install
   ```
2. (Valfritt) Skapa en `.env`-fil baserat på `.env.example` och fyll i dina Supabase-nycklar:
   ```bash
   VITE_SUPABASE_URL=https://<ditt-projekt>.supabase.co
   VITE_SUPABASE_ANON_KEY=<din-anon-key>
   ```
   Om du hoppar över `.env` kör appen i **demo-läge** med lokal dummy-data så att du kan se hela
   flödet utan backend.
